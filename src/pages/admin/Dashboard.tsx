...
const Dashboard: React.FC = () => {
    ...
    const StatCard = ({ title, value, subtext, icon, color }: { title: string, value: string | number, subtext: string, icon: React.ReactNode, color: string }) => (
        <div style={{
            ...
            <div>
                <p style={{ color: '#8c8c8c', fontSize: '0.8rem', marginBottom: '0.4rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{title}</p>
                <div style={{ fontSize: isMobile ? '1.4rem' : '1.8rem', fontWeight: 600, color: '#262626', lineHeight: 1.2 }}>{value}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', marginTop: '0.5rem', fontSize: '0.75rem' }}>
                    <ArrowUpRight size={12} color="#52c41a" />
    ...
    return (
        <div style={{ padding: isMobile ? '1rem' : '2rem', backgroundColor: '#f0f2f5', minHeight: '100%' }}>
            <div style={{ marginBottom: '2rem' }}>
                <h2 style={{ fontSize: isMobile ? '1.2rem' : '1.5rem', fontWeight: 600, color: '#262626', margin: 0 }}>Genel Bakış</h2>
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', fontSize: '0.8rem', color: '#8c8c8c', marginTop: '0.3rem' }}>
                    <span>Anasayfa</span>
                    <span>/</span>
                    <span>Dashboard</span>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
                <StatCard
                    title="Toplam Ürün"
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '2fr 1fr', gap: '1.5rem' }}>
                {/* Recent Orders Table */}
                <div style={{ backgroundColor: 'white', border: '1px solid #f0f0f0', borderRadius: '4px', padding: isMobile ? '1rem' : '1.5rem', overflow: 'hidden' }}>
                    ...
                    <div style={{ overflowX: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem', minWidth: isMobile ? '600px' : 'auto' }}>
                            <thead style={{ backgroundColor: '#fafafa' }}>
                                <tr>
                                    <th style={{ padding: '0.8rem', borderBottom: '1px solid #f0f0f0', color: '#595959', fontWeight: 600 }}>Müşteri</th>
                                    <th style={{ padding: '0.8rem', borderBottom: '1px solid #f0f0f0', color: '#595959', fontWeight: 600 }}>Tariح</th>
